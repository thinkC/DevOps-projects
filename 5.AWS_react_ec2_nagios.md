<!-- psql -d postgres -h mydatabase.c0mma8i2e22y.us-east-1.rds.amazonaws.com -p 5432 -U postgres -->

# Monitoring Linux Server on AWS with Nagios.

## What is Nagios?

The will install two servers server1 called _nagiosMonitoringServer_ will have nagios installed and server two will be called _linuxServer_.
We need to inatll _PHP_ and _Apache_ on the _nagiosMonitoringServer_.

<mark>I already assumed we have our ecs instance created. If not we can check out this tutorial on creating ec2 instance and connecting to it via ssh</mark>
...
![loging in to nagios](./img-aws-ec2-nagios/img1d.png)

![loging in to nagios](./img-aws-ec2-nagios/img1e.png).
...
![nagios host](./img-aws-ec2-nagios/img1f.png)

Click on Services to check the ping status.

![nagios ping](./img-aws-ec2-nagios/img1g.png)
...
![nagios ping](./img-aws-ec2-nagios/img1h.png)
![nagios ping](./img-aws-ec2-nagios/img1i.png)